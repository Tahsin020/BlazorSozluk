...
namespace BlazorSozluk.Infrastructure.Persistence.Context
{
    public class BlazorSozlukContext : DbContext
    {
        ...
        {
            IEnumerable<BaseEntity> addedEntites = ChangeTracker.Entries().Where(i => i.State == EntityState.Added).Select(i => (BaseEntity)i.Entity);
            PrepareAddedEntites(addedEntites);

        }
        private void PrepareAddedEntites(IEnumerable<BaseEntity> baseEntities)
        {
